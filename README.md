...
To try the plugin against a project without adding it as a dependency — for example, from a local checkout — use `uv run --with`. **Invoke pytest via `python -m pytest`, not the bare `pytest` command:**
